...
%% Montage
xyzz_img = (xyzz-min(xyzz,[],'all'))./max(xyzz-min(xyzz,[],'all'),[],'all'); xyzz_img = apply_CI(xyzz_img, CI_map, 0); % normalize xyz values to positive 0-1 
xyz_pos_img = mat2col(xyz_pos); 
xyz_pos_img_ci = apply_CI(xyz_pos_img, CI_map, 0); % normalize xyz values to positive 0-1
xyz_cleaned_img = mat2col(grain_props.xyz_cleaned);
xyz_cleaned_img_ci = apply_CI(xyz_cleaned_img, grain_props.CI, 0);
image_quality_img = mat2col(image_quality);

montage_figure = figure; montage({ebsd_img, image_quality_img, label2rgb(grain_props.BW), grain_props.ptc_map, mat2col(xyzz), xyz_pos_img, ...
