...
%   Additional Assitance:  Francois Usseglio-Viretta, NREL
    ...
    p = inputParser;
    addRequired(p, 'xyz');
%     addRequired(p, 'shape_type');
%     addRequired(p, 'subdivisions');
    ...
    xyz = p.Results.xyz;
%     shape_type = p.Results.shape_type;
%     subdivisions = p.Results.subdivisions;
...
%                 v_combos = combnk(faces(m,:), 2); % all possible index pairs - should only be 3
    ...
    for n = 1:length(counts) % per face on geometry
        ...
        elseif strcmp(plot_method, 'lines') % should be much simpler, just plot a line and its length % counts(n) ~= 0 && 
            face_vtx_indices =  faces(n,:);
            collected_vertices = vertices(face_vtx_indices(:), :); % per face
