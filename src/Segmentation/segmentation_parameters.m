...
bwshowlabels(seg_map_iq, 'first')
b_reg = input('Which # is the boundary region (1 or 2)?\n'); close all;
if b_reg == 2
    seg_map_iq = swap_labels(seg_map_iq,1,2); 
end
    
%% Determine which webs to keep/remove
figure; subplot(1,2,1); 
show_seg_boundary(seg_map_iq, 'ShowLabels', false);
title('All boundaries')

subplot(1,2,2); 
show_seg_boundary(seg_map_iq, 4);  % determine which webs to remove
title('4 Largest boundaries')

disp('Showing boundary regions')

inp = input('Input webbing numbers to keep ([1,2,66...]): '); close all;
[BW_iq_web, BW_cleaned_not_eroded] = clean_boundary(seg_map_iq, inp, struct_el);

%% Determine which grains to keep/remove
figure; show_seg_grains(seg_map_iq, 'ShowLabels', false); % all grains

inp2 = 1;
while inp2 == 1
    inp3 = input('Input area threshold (in um^2): '); close all; figure;
    thresholded_grain_lbls = show_seg_grains(seg_map_iq, round(inp3/(um_per_pix^2)), 'ShowLabels', false); % determine which grains to remove
    inp2 = input('1 to try again, enter only to move on: ');
end

%% Remove Grains by Threshold Using Above Input
BW_iq = clean_grains(seg_map_iq, thresholded_grain_lbls);% threshold grains, repalce with NaNs
...
figure; subplot(1,2,1)
show_seg_background(BW_iq_comb, 'ShowLabels', false);
title('All background')

subplot(1,2,2)
show_seg_background(BW_iq_comb, 5, 'ShowLabels', true);
...
BW_iq_comb_clean = background_to_boundary(BW_iq_comb, inp4);
...
figure;
BW_final = bwremovelabels(new_BW_iq, inp5); % remove labels and normalize remaining
